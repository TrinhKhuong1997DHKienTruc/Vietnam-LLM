...
def generate_price_state(stock_prices, end_index, window_size):
    ...
    '''
    # For early indices, we need to handle the case where we don't have enough history
    if end_index < window_size:
        # We don't have enough history, so we'll pad with the first available price
        # We need window_size + 1 prices total
        period = [stock_prices[0]] * (window_size + 1)
    else:
        # We have enough history
        start_index = end_index - window_size
        period = stock_prices[start_index:end_index+1]
    
    # Ensure we always have exactly window_size + 1 prices
    assert len(period) == window_size + 1, f"Expected {window_size + 1} prices, got {len(period)}"
...
def sharpe_ratio(returns, risk_free=0.0):
    ...
    Args:
        returns (array): Array of return rates
        risk_free (float): Risk-free rate (default: 0.0)
    ...
    # Calculate Sharpe ratio (annualized if daily returns)
