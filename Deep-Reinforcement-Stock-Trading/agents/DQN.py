...
class Agent(Portfolio):
    def __init__(self, state_dim, balance, is_eval=False, model_name=""):
        ...
        if is_eval:
            self.model = load_model('saved_models/{}.h5'.format(model_name), compile=False)
            # compile for predict compatibility
    ...
    def _prepare_state(self, state: np.ndarray) -> np.ndarray:
        # Handle legacy saved models that expect 3D inputs (batch, time, features)
        try:
            input_rank = len(self.model.inputs[0].shape)
        except Exception:
            input_rank = 2
        if input_rank == 3 and state.ndim == 2:
    ...
    def act(self, state):
        ...
        options = self.model.predict(self._prepare_state(state))
    ...
    def experience_replay(self):
        ...
        for state, actions, reward, next_state, done in mini_batch:
            if not done:
                Q_target_value = reward + self.gamma * np.amax(self.model.predict(self._prepare_state(next_state))[0])
            else:
                Q_target_value = reward
            next_actions = self.model.predict(self._prepare_state(state))
            next_actions[0][np.argmax(actions)] = Q_target_value
            history = self.model.fit(self._prepare_state(state), next_actions, epochs=1, verbose=0)
