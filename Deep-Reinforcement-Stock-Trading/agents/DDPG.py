...
# Simplified DDPG implementation for modern TensorFlow
class Agent(Portfolio):
    def __init__(self, state_dim, balance, is_eval=False, model_name=""):
        ...
        if is_eval and model_name:
            try:
                self.model = load_model(f'saved_models/{model_name}.h5')
            except:
                self.model = self.model()
        else:
            self.model = self.model()

        self.tensorboard = TensorBoard(log_dir='./logs/DDPG_tensorboard', update_freq=90)
