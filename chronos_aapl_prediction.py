...
def initialize_chronos_model():
    """Initialize the Chronos-Bolt full model"""
    print("Initializing Chronos-Bolt full model...")
    
    try:
        # Load the Chronos-Bolt full model (larger and more powerful)
        pipeline = ChronosPipeline.from_pretrained(
            "amazon/chronos-bolt-large",
            device_map="cuda" if torch.cuda.is_available() else "cpu",
            torch_dtype=torch.bfloat16 if torch.cuda.is_available() else torch.float32,
        )
        print(f"Chronos-Bolt Large model loaded successfully on {'CUDA' if torch.cuda.is_available() else 'CPU'}")
        return pipeline
        ...
        try:
            ...
            print(f"Chronos-Bolt Base model loaded successfully on {'CUDA' if torch.cuda.is_available() else 'CPU'}")
            return pipeline
            ...
            try:
                ...
                print("Chronos-Bolt Base model loaded with auto device mapping")
                return pipeline
            except Exception as e3:
                print(f"Failed to load any Chronos model: {e3}")
                raise
...
def create_visualization(train_data, test_data, predictions, lower_bound, upper_bound, save_path="demo_reports"):
    ...
    plt.title("NVDA Stock Price Prediction using Chronos-Bolt Full Model\nNext 14 Days Forecast")
    plt.legend()
...
def save_evaluation_report(metrics, predictions, save_path="demo_reports"):
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"Model: Amazon Chronos-Bolt Full (Large)\n")
...
def main():
    ...
    print("=" * 60)
    print("NVDA Stock Price Prediction using Amazon Chronos-Bolt Full Model")
    print("=" * 60)
